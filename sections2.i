  ...
  for (i = 0; i < numPhilosophers; i++)
    {
      ...
      mdat_mutex_init(philosophers[i], &chopsticks_mutex[i], ((void *)0));
    }
}
...
sectionRunPhilosopher(int philosopherID, int numRounds)
{
  ...
  int left = (((philosopherID)<((philosopherID + 1) % numPhil))?(philosopherID):((philosopherID + 1) % numPhil));

  int right = (((philosopherID)>((philosopherID + 1) % numPhil))?(philosopherID):((philosopherID + 1) % numPhil));
